...
using the `build_custom_table` function. These can be used and combined just
like other tables for discovery and prediction. Custom tables are built from 
a variant annotation (e.g. `Transcript_Strand`). The motifs can be provided 
manually using `data_factor` or will be generated automatically from the 
range of existing data in that column.
