...
# Setting up a musica object from variants
...
called. Using with_seed for reproducibility has the advantage of not
interfering with any other user seeds, but using one or the other is
important for several functions including *discover_signatures*,
*predict_exposure*, and *create_umap*, as these functions use stochastic
processes that may produce different results when run multiple times
