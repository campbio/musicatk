...
### Extract variants from files or objects
...
### Downloading variants for TCGA samples

For this tutorial, we will analyze mutational data from lung and skin cancer from TCGA. This data will be retrieved using the the `GDCquery` function from `r `BiocStyle::Biocpkg("TCGAbiolinks")` package. 

```{r get_tcga, message = FALSE}
...
To verify that the deconvolution algorithm produced good signatures, one strategy is to examine the patterns of mutations in individual samples with a high predicted percentage of a particular signature. If the shape of the counts match the patterns of the signature, then this is a good indicator that the deconvolution algorithms worked well. Counts for individual samples can be plotted with the `plot_sample_counts` function. For example, we can plot the sample with the highest proportion of the APOBEC signature:
...
## Prediction of COSMIC signatures
...
# Exploratory analysis of exposures

## Plotting exposures grouped by annotations

As mentioned previously, the `plot_exposures` function can plot sample exposures in a `musica_result` object. This function can group exposures by either a sample annotation or by a signature by setting the `group_by` parameter. To group by an annotation, the `groupBy` parameter must be set to `"annotation"` and the name of the annotation must be supplied via the `annotation` parameter. For example, the exposures from the previous result can be grouped by the `Tumor_Type` annotation:

```{r plot_exposures_by_subtype}
plot_exposures(result_cosmic_final, plot_type = "bar", group_by = "annotation", annotation = "Tumor_Type")
```
...
plot_exposures(result_cosmic_final, plot_type = "box", group_by = "annotation", annotation = "Tumor_Type")
```
...
plot_exposures(result_cosmic_final, plot_type = "box", group_by = "signature", 
               color_by = "annotation", annotation = "Tumor_Type")
```

## Visualize releationships between samples with 2-D embedding
...
## Cluster samples based on exposures
