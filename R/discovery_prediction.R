...
discover_signatures <- function(musica, table_name = NULL, num_signatures,
                                ...
                                par_cores = FALSE) {
  #if (!methods::is(input, "musica")) {
#    if (!methods::is(input, "matrix")) {
#      stop("Input to discover_signatures must be a musica object or a matrix")
#    }
#    musica <- methods::new("musica")
#    musica@count_tables@table_list[[1]] <- input
#    musica@count_tables@table_name[1] <- table_name
#    input <- musica
  #}
