...
test_that(desc = "Test kmeans visualization function", {
  ...
  p <- plot_cluster(result = res_annot, clusters = clust_out, group = "signature")
  expect_true(ggplot2::is.ggplot(p))
  p <- plot_cluster(result = res_annot, clusters = clust_out, group = "annotation", annotation = "Tumor_Subtypes")
  expect_true(ggplot2::is.ggplot(p))
  p <- plot_cluster(result = res_annot, clusters = clust_out, group = "none")
  expect_true(ggplot2::is.ggplot(p))
})