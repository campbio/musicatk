  ...
  if (input$SelectTable != "Custom") {
    ...
    if (input$SelectTable == "SBS192") {
      if (strand_type == "") {
        ...
        annotate_transcript_strand(vals$musica_contents, "19", build_table = T)
        return()
      } else {
        annotate_replication_strand(vals$musica_contents, rep_range, build_table = T)
        return()
      }
    }
    tryCatch( {
      ...
      #                    overwrite = T)
      build_standard_table(vals$musica_contents, vals$genome,
                           table_name = input$SelectTable, 
                           overwrite = T)
    },error = function(cond) {
